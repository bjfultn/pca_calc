...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Look for upgrade_definitions.json at the repo root
        repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))
        definitions_path = os.path.join(repo_root, 'upgrade_definitions.json')

        if not os.path.exists(definitions_path):
            self.stderr.write(
                f'Definitions file not found at {definitions_path}. '
